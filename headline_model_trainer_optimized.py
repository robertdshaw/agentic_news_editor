...
class CTRPredictor:
        ...
        for split, file_path in files.items():
            if file_path.exists():
                ...
                if "clicked" in df.columns:
                    ...
                    print(
                        f"  Class 0: {class_dist.get(0, 0)} ({class_pct.get(0, 0):.1f}%)"
                    )
                    print(
                        f"  Class 1: {class_dist.get(1, 0)} ({class_pct.get(1, 0):.1f}%)"
                    )
                else:
                    print(f"{split}: No 'clicked' or 'ctr' column found")

    def temporal_stratified_split(
        self, df, target_col="clicked", split_percentages=(0.6, 0.2, 0.2)
    ):
        ...
        if val_pos == 0 or test_pos == 0:
            ...
            print(
                "Falling back to stratified split to ensure balanced class distribution..."
            )
        ...
        for split_name, split_data in [
            ...
            if target_col in split_data.columns:
                ...
                print(
                    f"  {split_name}: {pos_count} positive ({pos_rate:.1f}%), {neg_count} negative"
                )
    ...
    def random_stratified_split(
        ...
        if len(positive_samples) < 3:
            ...
            if len(positive_samples) == 2:
                print(
                    "Using 1 positive sample each for train and val, leaving test without positive samples."
                )
                min_test_positive = 0
            elif len(positive_samples) == 1:
                print(
                    "Using 1 positive sample for train, leaving val and test without positive samples."
                )
                min_val_positive = 0
                min_test_positive = 0
        ...
        pos_train_size = max(
            len(positive_samples) - min_val_positive - min_test_positive, 1
        )
        ...
        neg_val = remaining_neg.sample(
            n=min(neg_val_size, len(remaining_neg)), random_state=42
        )
    ...
    def load_data_with_temporal_split(self, split_percentages=(0.6, 0.2, 0.2)):
        ...
        if train_path.exists():
            ...
            if combined_path.exists():
                ...
                return self.temporal_stratified_split(
    ...
    def extract_features(self, df):
        ...
        if "category" in df.columns:
            if "category" not in self.encoders:
                ...
                features["category_encoded"] = self.encoders["category"].fit_transform(
                    df["category"].fillna("unknown")
                )
            else:
                # Handle unseen categories
                try:
                    features["category_encoded"] = self.encoders["category"].transform(
                        df["category"].fillna("unknown")
                    )
                except ValueError:
                    # If new categories exist, fit_transform again
                    all_categories = list(self.encoders["category"].classes_) + list(
                        df["category"].fillna("unknown").unique()
                    )
        ...
        cache_file = (
            self.output_dir / f"embeddings_{len(df)}_{hash(str(df.index.tolist()))}.pkl"
        )
    ...
    def prepare_data(self, X, y, fit=True):
        ...
        if fit and y.mean() < 0.1:  # If very imbalanced
            logger.info(
                f"Applying SMOTE. Original distribution: {y.value_counts().to_dict()}"
            )
            ...
            try:
                ...
                logger.info(
                    f"After SMOTE+Undersampling: {pd.Series(y_final).value_counts().to_dict()}"
                )
    ...
    def train_models(self, X_train, y_train, X_val=None, y_val=None):
        ...
        for name, config in model_configs.items():
            ...
            if X_val is not None:
                ...
                self.results[name] = {
                    ...
                    "all_thresholds": list(
                        zip(pr_thresholds, precisions[:-1], recalls[:-1], f1_scores)
                    ),
                }

                logger.info(
                    f"{name} – val_auc: {auc:.4f}; "
        ...
        if self.best_model:
            logger.info(
                f"Selected best_model = {self.best_model} (AUC: {best_auc:.4f})"
            )

        return self.results

    def analyze_headline_pair(self, original, rewritten):
        """Analyze a pair of headlines and return comparison metrics"""
        from headline_metrics import HeadlineMetrics
        import logging

        try:
            # Create metrics analyzer if needed
            if not hasattr(self, "_metrics_analyzer"):
                self._metrics_analyzer = HeadlineMetrics(
                    model_path=str(self.output_dir / "ctr_model.pkl")
                )

            # Get comparison metrics
            comparison = self._metrics_analyzer.compare_headlines(original, rewritten)
            ...
            return {
                ...
                "improvement_category": self._get_improvement_category(
                    comparison["score_percent_change"]
                ),
                "key_improvements": ", ".join(comparison["key_improvements"]),
            }
        except Exception as e:
            logging.error(f"Error analyzing headline pair: {e}")
    ...
    def _get_improvement_category(self, percent_change):
        """Convert percentage improvement to a category name"""
    ...
    def evaluate_on_test(self, X_test, y_test):
        ...
        for threshold in thresholds:
            ...
            logger.info(
                f"Threshold {threshold}: Precision={precision:.4f}, Recall={recall:.4f}, F1={f1:.4f}"
            )
        ...
        detailed_report = {
            ...
            "classification_report": classification_report(
                y_test, best_results["predictions"], output_dict=True
            ),
            "confusion_matrix": confusion_matrix(
                y_test, best_results["predictions"]
            ).tolist(),
    ...
    def load_model(self, model_path=None):
        ...
        if "best_threshold" in model_data and model_data["best_threshold"]:
            self.results[self.best_model]["best_threshold"] = model_data[
                "best_threshold"
            ]
    ...
    def predict_click_probability(self, headlines, use_best_threshold=True):
            ...
            results.append(
                {
                    ...
                    "percentile": int(
                        100
                        - (
                            np.searchsorted(np.sort(probabilities), prob)
                            / len(probabilities)
                            * 100
                        )
                    ),
        ...
        if (
            use_best_threshold
            and self.best_model in self.results
            and "best_threshold" in self.results[self.best_model]
        ):
    ...
    def recreate_splits_with_proportions(self, split_percentages=(0.6, 0.2, 0.2)):
        ...
        train_data.to_csv(
            self.processed_data_dir / "train_headline_ctr.csv", index=False
        )
    ...
    def create_visualizations(self):
        ...
        try:
            ...
            plt.grid(True, alpha=0.3)
            plt.savefig(
                viz_dir / "prediction_distribution.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
            ...
            plt.grid(True, alpha=0.3)
            plt.savefig(
                viz_dir / "precision_recall_curve.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
            ...
            if (
                self.best_model in self.results
                and "all_thresholds" in self.results[self.best_model]
            ):
                ...
                plt.xscale("log")
                plt.savefig(
                    viz_dir / "threshold_analysis.png", dpi=300, bbox_inches="tight"
                )
                plt.close()
        ...
        for split, file_path in files.items():
            if file_path.exists():
                ...
                if "ctr" in df.columns:
                    ...
                    print(
                        f"  Zero CTR count: {(df['ctr'] == 0).sum()} ({(df['ctr'] == 0).mean()*100:.1f}%)"
                    )
                    print(
                        f"  Non-zero CTR count: {(df['ctr'] > 0).sum()} ({(df['ctr'] > 0).mean()*100:.1f}%)"
                    )
    ...
    def monitor_threshold_performance(self, threshold=0.05, time_period_days=7):
        ...
        logger.info(
            f"Monitoring threshold {threshold} performance over last {time_period_days} days"
        )
...
if __name__ == "__main__":
    ...
    if val_data is not None:
        ...
        if val_clicks == 0 or test_clicks == 0:
            train, val, test = ctr_predictor.recreate_splits_with_proportions(
                ...
                try:
                    ...
                    if results:
                        ...
                        for model_name, model_result in results.items():
                            ...
                            if "best_threshold" in model_result:
                                print(
                                    f"    Best threshold: {model_result['best_threshold']}"
                                )
                                print(f"    F1: {model_result['best_f1']:.4f}")
                                print(
                                    f"    Precision: {model_result['best_precision']:.4f}"
                                )
                        ...
                        if ctr_predictor.best_model:
                            ...
                            print(
                                f"Best threshold: {best_result.get('best_threshold', 'N/A')}"
                            )
                            print(f"F1: {best_result.get('best_f1', 'N/A'):.4f}")
                            print(
                                f"Precision: {best_result.get('best_precision', 'N/A'):.4f}"
                            )
                            print(
                                f"Recall: {best_result.get('best_recall', 'N/A'):.4f}"
                            )
                            print("\nInterpretation:")
                            ...
                            print(
                                "- Precision: Of predicted clicks, what % were actual clicks"
                            )
                            print(
                                "- Recall: Of actual clicks, what % were predicted correctly"
                            )
                except Exception as e:
                    ...
                    if ctr_predictor.results:
                        ...
                        for model_name, result in ctr_predictor.results.items():
                            if result.get("auc") is not None:
                                ...
                                if "best_threshold" in result:
                                    ...
                                    print(
                                        f"  Precision: {result['best_precision']:.4f}"
                                    )
                        ...
                        ctr_predictor.best_model = max(
                            [
                                k
                                for k, v in ctr_predictor.results.items()
    ...
    if ctr_predictor.best_model and ctr_predictor.results:
        ...
        predictions_all = ctr_predictor.predict_click_probability(
            test_headlines, use_best_threshold=False
        )
        predictions_filtered = ctr_predictor.predict_click_probability(
            test_headlines, use_best_threshold=True
        )
        best_threshold = ctr_predictor.results[ctr_predictor.best_model].get(
            "best_threshold", None
        )
        ...
        if predictions_all:
            ...
            if len(predictions_filtered) == 0:
                ...
                for pred in predictions_all[:3]:
                    ...
                    print(
                        f"   Probability: {pred['click_probability']:.6f} ({pred['percentile']}th percentile)"
                    )
                    print()
                ...
                for pred in predictions_filtered:
                    print(f"Rank {pred['rank']:2d}: {pred['headline']}")
                    print(
                        f"         Click Probability: {pred['click_probability']:.6f}"
                    )
                    print(f"         Relative Rank: {pred['relative_rank']}")
                    print()
            else:
            ...
            print("\n=== Dataset Insights ===")
            print(
                f"- Training data: ~{len(ctr_predictor.results[ctr_predictor.best_model].get('predictions', [])):,} samples"
            )
            print(f"- Positive rate: Very low (~1-2%)")
            print(
                f"- Model achieved: {ctr_predictor.results[ctr_predictor.best_model]['auc']:.4f} AUC"
            )
            if "best_threshold" in ctr_predictor.results[ctr_predictor.best_model]:
                print(
                    f"- Best threshold: {ctr_predictor.results[ctr_predictor.best_model]['best_threshold']:.6f}"
                )
            print(
