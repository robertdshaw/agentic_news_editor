...
class HeadlineLearningLoop:
    ...
    def prompt_improvement_report(self):
        ...
        if len(self.data) < 20:
            st.warning(
                "Not enough headline data for a meaningful report (minimum 20 required)"
            )
            return False
...
headline_learner = HeadlineLearningLoop(
    data_file="headline_learning_data.csv", model_path="model_output/ctr_model.pkl"
)
...
def analyze_headline_effectiveness(df, openai_client=None):
    ...
    for col in [
        ...
        df[col] = 0 if "score" in col or "ctr" in col or "improvement" in col else ""

    # Process each row inside a try/except
    for i, row in df.iterrows():
        ...
        try:
            if ctr_predictor:
                orig_ctr = ctr_predictor.predict_ctr(row["title"])
                new_ctr = ctr_predictor.predict_ctr(row["rewritten_title"])
                improvement = (
                    ((new_ctr - orig_ctr) / orig_ctr * 100)
                    if orig_ctr > 0
                    else (100 if new_ctr > 0 else 0)
                )
                comparison = metrics_analyzer.compare_headlines(
                    row["title"], row["rewritten_title"]
                )
                key_factors = comparison.get(
                    "key_improvements", ["CTR-based improvement"]
                )

                df.at[i, "headline_score_original"] = orig_ctr
                df.at[i, "headline_score_rewritten"] = new_ctr
                df.at[i, "headline_ctr_original"] = orig_ctr * 100
                df.at[i, "headline_ctr_rewritten"] = new_ctr * 100
                ...
                factors = comparison.get("key_improvements", [])
                df.at[i, "headline_key_factors"] = (
                    ", ".join(factors)
                    if factors
...
def calculate_evaluation_metrics(curated_articles):
    ...
    headline_metrics = {
        ...
        "avg_ctr_change": (
            curated_articles["headline_ctr_rewritten"]
            - curated_articles["headline_ctr_original"]
        ).mean(),
    }
...
def display_headline_with_metrics(original_title, rewritten_title, metrics=None):
    """Display original and rewritten titles with CTR-based improvement metrics"""
    st.markdown(
        f"""
    <div class="title-comparison">
        <div class="original-headline">Original: {original_title}</div>
        ...
        if improvement != 0:
            ...
            <div class="headline-metrics">
                <div class="metrics-row">
                    ...
                    <span class="metric-value">{ctr_original:.3f}% → {ctr_rewritten:.3f}%</span>
...
def add_headline_testing_section():
    """Add a section for testing individual headlines"""
    st.sidebar.markdown("---")
    st.sidebar.subheader("🧪 Test Headline Effectiveness")

    ctr_predictor = st.session_state.get("ctr_predictor")

    if ctr_predictor:
        original_headline = st.sidebar.text_area(
            "Original Headline", placeholder="Enter the original headline...", height=60
        )

        rewritten_headline = st.sidebar.text_area(
            "Rewritten Headline",
            placeholder="Enter the rewritten headline...",
            height=60,
        )

        if original_headline and rewritten_headline:
            if st.sidebar.button("Compare Headlines", use_container_width=True):
                try:
                    # Get CTR predictions
                    original_ctr = ctr_predictor.predict_ctr(original_headline)
                    rewritten_ctr = ctr_predictor.predict_ctr(rewritten_headline)

                    # Calculate improvement
                    if original_ctr > 0:
                        improvement = (
                            (rewritten_ctr - original_ctr) / original_ctr * 100
                        )
                    else:
                    ...
                    col1.metric("Original CTR", f"{original_ctr*100:.3f}%")
                    col2.metric("Rewritten CTR", f"{rewritten_ctr*100:.3f}%")

                    # Show improvement
                    if improvement > 0:
                        st.sidebar.success(f"✅ Improved by {improvement:.1f}%")
                    else:
                        st.sidebar.error(f"❌ Decreased by {abs(improvement):.1f}%")

                    # Get recommendation
                    recommendation = ctr_predictor.get_threshold_recommendation(
                        rewritten_ctr
                    )
                    st.sidebar.info(f"Recommendation: {recommendation['action']}")

                    # Add to learning system
                    if st.sidebar.button("Add to Learning System"):
                        if headline_learner.add_headline_pair(
                            original_headline, rewritten_headline
                        ):
                            st.sidebar.success("✅ Added to learning system!")
                        else:
                            st.sidebar.error("❌ Failed to add to learning system")

                except Exception as e:
                    st.sidebar.error(f"Error comparing headlines: {e}")
    else:
        st.sidebar.info("CTR model not available. Please check model files.")
    ...
    if st.button("Generate Research Report", use_container_width=True):
        try:
            if (
                ...
                st.metric(
                    "Improvement Rate",
                    f"{metrics['headline_metrics']['improvement_rate']:.1%}",
                )
                st.metric(
                    "Avg CTR Change",
                    f"{metrics['headline_metrics']['avg_ctr_change']:.1f}%",
                )
...
def display_featured_article(articles_df, show_comparison):
    ...
    if len(articles_df) > 0:
        ...
        with st.container():
            ...
            if show_comparison:
                if (
                    "headline_improvement" in main_article
                    and "headline_ctr_original" in main_article
                ):
                    display_headline_with_metrics(
                        main_article["original_title"],
                        main_article["rewritten_title"],
                        {
                            "headline_improvement": main_article.get(
                                "headline_improvement", 0
                            ),
                            "headline_ctr_original": main_article.get(
                                "headline_ctr_original", 0
                            ),
                            "headline_ctr_rewritten": main_article.get(
                                "headline_ctr_rewritten", 0
                            ),
                            "headline_key_factors": main_article.get(
                                "headline_key_factors", ""
                            ),
                        },
                    )
                else:
                    display_headline_comparison(
                        main_article["original_title"], main_article["rewritten_title"]
                    )
            else:
...
def display_topic_sections(articles_df, show_comparison):
    ...
    remaining_articles = (
        articles_df.iloc[7:] if len(articles_df) > 7 else pd.DataFrame()
    )
    ...
    if len(remaining_articles) > 0:
        ...
        for topic in topics:
            st.markdown(
                f'<h2 class="section-title">{topic.upper()}</h2>',
                unsafe_allow_html=True,
            )
